...
class SignupVC: BaseVC, UITextFieldDelegate, UITextViewDelegate, SCImagePickerDelegate {
    ...
    private func performSignup(completion:((_ flag: Bool) -> Void)?) {
    ...
    @IBAction func btnSignUpTap(_ sender: Any) {
        ...
        delayInLoading {
            ...
            let controller = NavigationManager.shared.selectCategoriesVC
            self.push(controller: controller)
            
            /*self.performSignup { (flag: Bool) in
                if flag {
